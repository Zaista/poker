$(document).ready(function() {
	...
	function startGame() {
		...
		setFirstFive = setInterval(function() {
			...
			if (counter == 5) {
				...
				drawCard();
				$(".open .card-back").css("background-image", "url(images/cards/" + cards[activeCard++] + ".png");
				setDraggable($(".open"));
				goTo($(".open"), $("#free-spot"));
			}
		}, 200);
	 ...
	 */
	function drawCard() {
		$(".static").parent().append(
			'	<section class="cards open" style="margin-left: -' + margin + 'px">' +
			'		<div class="card">' +
	...
	function endGame(hands) {
		...
		if (scoreboard.length < 10 || totalScore > scoreboard[scoreboard.length - 1][1]) {
	...
	function rankHand(hand) {
		...
		switch (bucket.length) {
			case 4:
				return 1;
			case 3:
				if (bucket[0].length === 3 || bucket[1].length === 3)
					return 3;
				else return 2;
			case 2:
				if (bucket[0].length === 3 || bucket[1].length === 3)
